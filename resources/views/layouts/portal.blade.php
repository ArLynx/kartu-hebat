...
<body class="min-h-screen bg-[#f8f7fb]">
    ...
    <aside class="fixed inset-y-0 left-0 z-50 flex w-64 -translate-x-full flex-col bg-navy-950 px-4 py-5 text-white transition-transform duration-200 lg:translate-x-0"
        ...
        <nav class="mt-10 flex-1 space-y-1.5 overflow-y-auto">
            @if($isStudent)
                <x-sidebar-link :href="route('mahasiswa.dashboard')" :active="request()->routeIs('mahasiswa.dashboard')" icon="home">Beranda Beasiswa</x-sidebar-link>
                <x-sidebar-link :href="route('mahasiswa.pendaftaran.create')" :active="request()->routeIs('mahasiswa.pendaftaran.*')" icon="form">Buat Pendaftaran</x-sidebar-link>
                <x-sidebar-link :href="route('mahasiswa.data-pribadi.index')" :active="request()->routeIs('mahasiswa.data-pribadi.*')" icon="user">Data Pribadi</x-sidebar-link>
                <x-sidebar-link :href="route('mahasiswa.pendidikan.index')" :active="request()->routeIs('mahasiswa.pendidikan.*')" icon="building">Pendidikan</x-sidebar-link>
                <x-sidebar-link :href="route('mahasiswa.prestasi.index')" :active="request()->routeIs('mahasiswa.prestasi.*')" icon="chart">Prestasi</x-sidebar-link>
                <x-sidebar-link :href="route('mahasiswa.orang-tua.index')" :active="request()->routeIs('mahasiswa.orang-tua.*')" icon="users">Orang Tua</x-sidebar-link>
                <x-sidebar-link :href="route('mahasiswa.dokumen.index')" :active="request()->routeIs('mahasiswa.dokumen.*')" icon="folder">Dokumen</x-sidebar-link>
                <x-sidebar-link :href="route('mahasiswa.review.index')" :active="request()->routeIs('mahasiswa.review.*', 'mahasiswa.submit.*')" icon="check">Review & Submit</x-sidebar-link>
                <x-sidebar-link :href="route('public.results')" :active="request()->routeIs('public.results')" icon="announcement">Pengumuman</x-sidebar-link>
            ...
            @else
                ...
                @if($user->hasRole('operator_kabupaten'))
                    <x-sidebar-link :href="route('operator.reconciliation')" :active="request()->routeIs('operator.reconciliation')" icon="shield">Rekonsiliasi Dinas</x-sidebar-link>
                    <x-sidebar-link :href="route('operator.selection')" :active="request()->routeIs('operator.selection*')" icon="users">Penetapan Penerima</x-sidebar-link>
            ...
            <x-sidebar-link :href="route('notifications.index')" :active="request()->routeIs('notifications.*')" icon="bell">
                ...
                @if($unreadNotifications > 0)
                    <span class="ml-auto rounded-full bg-red-500 px-2 py-0.5 text-[10px] text-white">{{ $unreadNotifications }}</span>
                @endif
            </x-sidebar-link>
        ...
        <header class="sticky top-0 z-30 border-b border-slate-200 bg-white/95 backdrop-blur">
            <div class="flex h-16 items-center gap-4 px-4 sm:px-6 lg:px-8">
                ...
                <div class="ml-auto flex items-center gap-3">
                    <a href="{{ route('notifications.index') }}" class="relative rounded-full p-2 text-slate-600 hover:bg-slate-100">
                        <x-icon name="bell" />
                        @if($unreadNotifications > 0)
                            <span class="absolute right-1 top-1 h-2.5 w-2.5 rounded-full border-2 border-white bg-red-500"></span>
                        @endif
                    </a>
                    <div class="hidden h-8 w-px bg-slate-200 sm:block"></div>
                    <div class="flex items-center gap-3">
                        <div class="flex h-9 w-9 items-center justify-center rounded-full bg-brand-100 font-bold text-brand-700">
