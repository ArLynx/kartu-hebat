...
<div class="flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between">
    ...
    @if($pendaftaran)
        <div class="text-left sm:text-right">
            <span class="inline-flex rounded-full border border-blue-200 bg-blue-50 px-4 py-2 text-sm font-semibold text-brand-700">{{ $pendaftaran->nomor_pendaftaran }}</span>
            <p class="mt-2 text-sm font-medium text-slate-700">{{ $pendaftaran->application?->status?->label() ?? ucfirst($pendaftaran->status) }}</p>
        </div>
    @endif
</div>
...
@if(!$pendaftaran)
    <section class="card mt-7 p-7 sm:p-10">
        <div class="flex flex-col gap-7 lg:flex-row lg:items-center lg:justify-between">
            <div class="max-w-2xl">
                <div class="flex h-14 w-14 items-center justify-center rounded-xl bg-brand-100 text-brand-700"><x-icon name="form" class="h-7 w-7" /></div>
                <h2 class="mt-5 text-2xl font-extrabold">Belum Ada Pendaftaran Beasiswa</h2>
                <p class="mt-3 leading-7 text-slate-600">Buat draft pendaftaran, pilih kategori beasiswa, lalu lengkapi data sesuai tujuh tahapan pendaftaran.</p>
            </div>
            <a href="{{ route('mahasiswa.pendaftaran.create') }}" class="btn-primary justify-center"><x-icon name="form" class="h-5 w-5" />Buat Pendaftaran</a>
        </div>
    </section>
@else
    @php
        ...
        <section class="card mt-7 overflow-hidden">
            <div class="border-b border-slate-200 px-6 py-5 sm:px-8">
                ...
                <div class="mt-2 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        ...
                        <p class="mt-1 text-sm text-slate-600">Nomor pengajuan: {{ $pendaftaran->application->nomor_pengajuan }}</p>
                    </div>
            ...
            <div class="px-6 py-5 sm:px-8">
                <div class="h-2 overflow-hidden rounded-full bg-slate-100">
                    <div class="h-full rounded-full bg-brand-600" style="width: {{ $pendaftaran->application->status->progress() }}%"></div>
                </div>
                ...
                @if(in_array($pendaftaran->status, ['revision'], true))
                    <p class="mt-4 text-sm text-slate-600">Perbaiki data yang diminta, konfirmasi ulang tahap review, kemudian kirim kembali pendaftaran.</p>
                @endif
                        ...
                        <div class="mt-3 space-y-3">
                            @foreach($pendaftaran->application->verificationLogs->take(3) as $log)
                                <div class="flex items-start justify-between gap-4 text-sm">
                                    ...
                                    <p class="shrink-0 text-xs text-slate-500">{{ $log->created_at->translatedFormat('d M Y, H:i') }}</p>
                                </div>
                            @endforeach
                        </div>
                    </div>
    ...
    <section class="card mt-7 p-6 sm:p-8">
        <div class="flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between">
            <div>
                ...
                <p class="mt-2 text-sm text-slate-600">Kategori: <strong>{{ $pendaftaran->kategoriBeasiswa?->nama }}</strong></p>
            </div>
            <span class="inline-flex w-fit rounded-full {{ $stepStatuses[7] ? 'bg-emerald-100 text-emerald-800' : 'bg-amber-100 text-amber-800' }} px-4 py-2 text-sm font-bold">{{ ucfirst($pendaftaran->status) }}</span>
        </div>
            ...
            @foreach($steps as $number => $step)
                ...
                <a href="{{ route($step['route']) }}" @class([
                    ...
                    <div @class([
                        ...
                        @if($completed)<x-icon name="check" class="h-4 w-4" />@else{{ $number }}@endif
                    </div>
